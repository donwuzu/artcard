...
<x-app-layout>
   ...
   <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const detailSections = document.querySelectorAll('[id^="items-"]');
    ...
    function toggleOrderDetails(orderId) {
        const target = document.getElementById(`items-${orderId}`);
        if (!target) return;

        const isHidden = target.classList.contains('max-h-0');

        document.querySelectorAll('[id^="items-"]').forEach(section => {
