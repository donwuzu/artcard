    ...
    <table class="w-full table-auto divide-y divide-gray-300 text-xs sm:text-sm">
        <thead class="bg-gray-100 font-semibold text-gray-700">
            <tr>
                ...
                <th class="px-1 sm:px-4 py-3 whitespace-nowrap sticky left-0 bg-gray-100 z-10 w-[100px] sm:w-auto">Name</th>
            ...
            @foreach ($filteredOrders as $index => $order)
                <tr class="hover:bg-gray-50 cursor-pointer"
                  onclick="toggleOrderDetails({{ $order->id }})">
                    ...
                    <td class="px-1 sm:px-4 py-3 sticky left-0 bg-white z-0 w-[100px] sm:w-auto truncate">{{ $order->name }}</td>
                    <td class="px-2 py-3 sm:px-4">{{ $order->phone }}</td>
                    <td class="px-2 py-3 sm:px-4">{{ $order->location }}</td>
                    <td class="px-2 py-3 sm:px-4">KSh {{ number_format($order->total_price) }}</td>
                    <td class="px-2 py-3 sm:px-4">{{ $order->created_at->format('d M Y') }}</td>
                    <td class="px-2 py-3 sm:px-4">{{ $order->status ?? 'unpaid' }}</td>
                    <td class="px-2 py-3 sm:px-4">
                        <form action="{{ route('expenses.toggleStatus', $order->id) }}" method="POST">
                            @csrf
                            <button class="{{ $order->status === 'paid' ? 'bg-black' : 'bg-green-600' }} text-white px-3 py-1 rounded">
                    ...
                    <td class="px-2 py-3 sm:px-4">
                        <form action="{{ route('expenses.destroy', $order->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this order?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-600 hover:bg-red-800 text-white px-3 py-1 rounded">
                                Delete
                            </button>
                        </form>
                    </td>
