<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
         ...
         <form method="POST" action="{{ route('admin.clocks.store') }}" enctype="multipart/form-data" class="bg-white shadow p-6 rounded-xl space-y-6">
    @csrf

    <div x-data="{ previewUrl: null }" class="space-y-2">
        <label for="clock" class="block text-sm font-medium text-gray-700 mb-2">Upload Clock</label>
        <label for="clock" class="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100">
            ...
            <input id="clock" name="clock" type="file" class="hidden" required @change="const file = $event.target.files[0]; if (file) previewUrl = URL.createObjectURL(file);" />
        </label>
    </div>
